...
fn main() {
    ...
    let plain: usize = data.iter().map(|d| base91_jdp::encode(d).len()).sum();
    ...
    for level in [-5i32, 3, 19] {
        let z: usize = data.iter().map(|d| base91_jdp::encode_zstd(d, level).unwrap().len()).sum();
        let a: usize = data.iter().map(|d| base91_jdp::encode_auto(d, level).unwrap().len()).sum();
